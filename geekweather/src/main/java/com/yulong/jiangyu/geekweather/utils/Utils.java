...
public class Utils {
     ...
     * @return 返回解析好的天气信息
     */
    public static WthrcdnWeatherEntity getWthrcdnWeatherEntity(InputStream inputStream) {
        //初始化
        WthrcdnWeatherEntity wthrcdnWeatherEntity = new WthrcdnWeatherEntity();
        WthrcdnWeatherForecastDaysEntity wthrcdnWeatherForecastDaysEntity = null;
        List<WthrcdnWeatherForecastDaysEntity> wthrcdnWeatherForecastDaysEntityList = new ArrayList<>();
        LifeIndexEntity lifeIndexEntity = null;
        List<LifeIndexEntity> lifeIndexEntityList = new ArrayList<>();
        //是否是多天天气预报
                ...
                switch (eventType) {
                    case XmlPullParser.START_TAG:
                        switch (parser.getName()) {
                            case "city"://城市
                                wthrcdnWeatherEntity.setmCity(parser.nextText());
                                break;
                            case "updatetime"://更新时间
                                wthrcdnWeatherEntity.setmUpdateTime(parser.nextText());
                                break;
                            case "wendu"://当前温度
                                wthrcdnWeatherEntity.setmTemperature(parser.nextText());
                                break;
                            case "fengli"://风力
                                if (!isDaysForecast)
                                    wthrcdnWeatherEntity.setmWindPower(parser.nextText());
                                else if (wthrcdnWeatherForecastDaysEntity != null) {
                                    if (isDay)
                                        wthrcdnWeatherForecastDaysEntity.setmWindPowerDay(parser.nextText());
                                    else
                                        wthrcdnWeatherForecastDaysEntity.setmWindPowerNight(parser.nextText());
                                }
                                break;
                            case "shidu"://湿度
                                wthrcdnWeatherEntity.setmHumidity(parser.nextText());
                                break;
                            case "fengxiang"://风向
                                if (!isDaysForecast)
                                    wthrcdnWeatherEntity.setmWindDirection(parser.nextText());
                                else if (wthrcdnWeatherForecastDaysEntity != null) {
                                    if (isDay)
                                        wthrcdnWeatherForecastDaysEntity.setmWindDirectionDay(parser.nextText());
                                    else
                                        wthrcdnWeatherForecastDaysEntity.setmWindDirectionNight(parser.nextText());
                                }
                                break;
                            case "sunrise_1"://日出时间
                                wthrcdnWeatherEntity.setmSunrise(parser.nextText());
                                break;
                            case "sunset_1"://日落时间
                                wthrcdnWeatherEntity.setmSunset(parser.nextText());
                                break;
                            case "aqi"://空气质量指标
                                wthrcdnWeatherEntity.setmAQI(parser.nextText());
                                break;
                            case "quality"://空气质量
                                wthrcdnWeatherEntity.setmAQ(parser.nextText());
                                break;
                            case "alarmType"://警报类型
                                wthrcdnWeatherEntity.setmAlarmType(parser.nextText());
                                break;
                            case "alarmDegree"://警报等级
                                wthrcdnWeatherEntity.setmAlarmDegree(parser.nextText());
                                break;
                            case "alarmText"://警报内容
                                wthrcdnWeatherEntity.setmAlarmText(parser.nextText());
                                break;
                            case "alarm_details"://警报详情
                                wthrcdnWeatherEntity.setmAlarmDetail(parser.nextText());
                                break;
                            case "time"://警报时间
                                wthrcdnWeatherEntity.setmAlarmTime(parser.nextText());
                                break;
                            ...
                            case "yesterday"://昨日天气
                                isDaysForecast = true;
                                wthrcdnWeatherForecastDaysEntity = new WthrcdnWeatherForecastDaysEntity();
                                break;
                            case "date_1"://日期
                                if (wthrcdnWeatherForecastDaysEntity != null)
                                    wthrcdnWeatherForecastDaysEntity.setmDate(parser.nextText());
                                break;
                            case "high_1"://高温
                                if (wthrcdnWeatherForecastDaysEntity != null)
                                    wthrcdnWeatherForecastDaysEntity.setmHighTemperature(parser.nextText());
                                break;
                            case "low_1"://低温
                                if (wthrcdnWeatherForecastDaysEntity != null)
                                    wthrcdnWeatherForecastDaysEntity.setmLowTemperature(parser.nextText());
                                break;
                            case "day_1"://白天
                                isDay = true;
                                break;
                            ...
                            case "type_1"://天气类型
                                if (wthrcdnWeatherForecastDaysEntity != null) {
                                    if (isDay)
                                        wthrcdnWeatherForecastDaysEntity.setmTypeDay(parser.nextText());
                                    else
                                        wthrcdnWeatherForecastDaysEntity.setmTypeNight(parser.nextText());
                                }
                                break;
                            case "fx_1"://风向
                                if (wthrcdnWeatherForecastDaysEntity != null) {
                            ...
                            case "type"://类型
                                ...
                                break;
                            case "zhishu"://生活指数
                                lifeIndexEntity = new LifeIndexEntity();
                                break;
                            case "name"://指数名
                                if (lifeIndexEntity != null)
                                    lifeIndexEntity.setmIndexName(parser.nextText());
                                break;
                            case "value"://指数内容
                                if (lifeIndexEntity != null)
                                    lifeIndexEntity.setmIndexSuggestion(parser.nextText());
                                break;
                            case "detail"://指数详情
                                if (lifeIndexEntity != null)
                                    lifeIndexEntity.setmIndexDetail(parser.nextText());
                                break;
                        }
                        break;
                    case XmlPullParser.END_TAG:
                        switch (parser.getName()) {
                            case "yesterday"://昨天天气
                            case "weather"://多天天气
                                wthrcdnWeatherForecastDaysEntityList.add(wthrcdnWeatherForecastDaysEntity);
                                wthrcdnWeatherForecastDaysEntity = null;
                                break;
                            case "zhishu"://生活指数
                                lifeIndexEntityList.add(lifeIndexEntity);
                                lifeIndexEntity = null;
                                break;
                        }
                        break;
                }
        ...
        wthrcdnWeatherEntity.setmWeatherDaysForecasts(wthrcdnWeatherForecastDaysEntityList);
        wthrcdnWeatherEntity.setmWeatherLifeIndies(lifeIndexEntityList);
        return wthrcdnWeatherEntity;
    }
