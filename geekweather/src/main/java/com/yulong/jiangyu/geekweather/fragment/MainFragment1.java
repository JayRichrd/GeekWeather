...
import com.yulong.jiangyu.geekweather.activity.CityManageActivity1;
...
import com.yulong.jiangyu.geekweather.listener.BaiDuLocationListener;
...
public class MainFragment1 extends Fragment implements IView {
    //日志TAG
                        ...
                        switch (item.getItemId()) {
                            ...
                            case R.id.item_version:
                                ...
                                try {
                                    ...
                                    Log.e(TAG, getString(R.string.log_failed_to_get_application_version));
                                }
                                break;
                            default:
                        }
    ...
    private void getData() {
        mWeatherPresenter.requestWeather(mCity, false);
        mDatePresenter.requestDate(Utils.getCurrentDate(getString(R.string.format_request_date)), false);
        Log.d(TAG, "===" + getString(R.string.main_fragment_toast_update_start) + "===");
        Toast.makeText(getActivity(), getString(R.string.main_fragment_toast_update_start), Toast
                .LENGTH_SHORT).show();
    }
     ...
     * @param calendar                当前日历
     */
    private void setDaysForecast(List<WthrcdnWeatherForecastDaysEntity> weatherForecastDaysList, Calendar calendar,
                                 int hour) {
        //星期几
        ...
        for (WthrcdnWeatherForecastDaysEntity weatherForecastDaysEntity : weatherForecastDaysList) {
     ...
     */
    private void setWeatherType(int hour, WthrcdnWeatherForecastDaysEntity weatherForecastDaysEntity) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.iv_add_city:
                Intent intent = new Intent(getActivity(), CityManageActivity1.class);
    ...
    public void updateWeather(IDataEntity dataEntity) {
        ...
        if (ptrsvRoot.isRefreshing())
            stopRefresh();
        Toast.makeText(getActivity(), getString(R.string.main_fragment_toast_update_finished),
                Toast.LENGTH_SHORT).show();

        // 先将数据转换成目标数据
        WthrcdnWeatherEntity wthrcdnWeatherEntity = (WthrcdnWeatherEntity) dataEntity;

        // 设置城市
        tvCity.setText(wthrcdnWeatherEntity.getmCity());
        // 更新头部时间
        tvUpdateTime.setText(String.format(getString(R.string.format_update_time), wthrcdnWeatherEntity
                .getmUpdateTime()));

        List<WthrcdnWeatherForecastDaysEntity> weatherForecastDaysList = wthrcdnWeatherEntity
                .getmWeatherDaysForecasts();
        //当前时间的小时数
        Calendar calendar = Calendar.getInstance();
        int hour = calendar.get(Calendar.HOUR_OF_DAY);
        setTemperature(wthrcdnWeatherEntity);
        setWeatherType(hour, weatherForecastDaysList.get(1));
        tvWind.setText(wthrcdnWeatherEntity.getmWindDirection() + " " + wthrcdnWeatherEntity.getmWindPower());
        tvHumidity.setText(String.format(getString(R.string.format_humidity), wthrcdnWeatherEntity.getmHumidity()));
        setAqi(wthrcdnWeatherEntity);
        //设置未来几天天气
        if (6 == weatherForecastDaysList.size()) {
            setDaysForecast(weatherForecastDaysList, calendar, hour);
        }
        // 更新生活指数信息
        setLifeIndex(wthrcdnWeatherEntity.getmWeatherLifeIndies());
    }
    ...
    public void updateDate(IDataEntity dataEntity) {
        ...
        // 现将数据转换成目标数据
        JuHeDateEntity juHeDateEntity = (JuHeDateEntity) dataEntity;

        String formatDate = Utils.getCurrentDate(getString(R.string.format_ui_date));
        String lunarStr = juHeDateEntity.getResult().getData().getLunar();
        tvDate.setText(String.format(getString(R.string.format_date), formatDate, lunarStr));
    }
