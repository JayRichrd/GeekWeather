...
public class CityManageAdapter extends ArrayAdapter<CityManageEntity> {
    ...
    public CityManageAdapter(@NonNull Context context, @LayoutRes int resource, @NonNull List<CityManageEntity>
        ...
        // 获取默认城市
        mSharedPreferences = mContext.getSharedPreferences(Constant.SHARED_PREFERENCE, Context.MODE_PRIVATE);
        ...
        if (1 == mCityList.size()) { // 城市列表为空
            ...
            if (position != mCityList.size() - 1) {// 非最后一项——添加城市
                        ...
                        public void onClick(View v) {
                            if (cityName != null && cityName.equals(mDefaultCity)) {
                                Toast.makeText(mContext, mContext.getString(
                                        R.string.city_manage_activity_toast_delete_default_city), Toast.LENGTH_SHORT)
                                        .show();
                            } else {
                                // 删除并刷新
                ...
                if (cityName != null && mDefaultCity.equals(cityName)) {
                    viewHolder.tvDefault.setText(R.string.city_manage_activity_tv_default_my_default);
                    viewHolder.tvDefault.setBackground(mContext.getDrawable(R.drawable.bg_gv_city_manage_default));
                } else {
                    viewHolder.tvDefault.setText(R.string.city_manage_activity_tv_default_set_default);
                ...
                viewHolder.tvDefault.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (cityName != null && cityName.equals(mDefaultCity)) {
                            Toast.makeText(mContext, mContext.getString(
                                    R.string.city_manage_activity_toast_already_default_city), Toast.LENGTH_SHORT)
                                    .show();
                        } else {
