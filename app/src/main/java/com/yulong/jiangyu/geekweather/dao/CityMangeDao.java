...
public class CityMangeDao {
    Context mContext;
    CityManageDBHelper helper;
    Dao<CityManage, Integer> mDao;

    public CityMangeDao(Context mContext) {
        this.mContext = mContext;
        helper = CityManageDBHelper.getInstance(mContext);
        try {
            mDao = helper.getDao(CityManage.class);
     ...
     */
    public boolean insert(CityManage cityManage) {
     ...
     * @return 返回数据库表中的全部数据
     */
    public List<CityManage> queryAll() {
        ArrayList<CityManage> cityManageList = null;
        try {
            cityManageList = (ArrayList<CityManage>) mDao.queryForAll();
        } catch (SQLException e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    public boolean update(CityManage cityManage) {
        boolean result = false;
        try {
            DeleteBuilder deleteBuilder = mDao.deleteBuilder();
